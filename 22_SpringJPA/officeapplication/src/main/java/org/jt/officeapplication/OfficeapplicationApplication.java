...
public class OfficeapplicationApplication implements CommandLineRunner {
	...
	public void run(String... args) throws Exception {
		...
		var optionalStudent = studentRepository.findById(202);
		if (optionalStudent.isPresent()) {
			var student = optionalStudent.get();
			System.out.println(student.getName());
			System.out.println(student.getRoll());
			System.out.println(student.getVerfication().getCity());
		} else {
	...
	private void addStudent() {
		...
		var verfication = Verfication.builder()
				.addharNumber("987654321")
				.city("kerla")
				.state("kerla")
				.build();

		student.setVerfication(verfication);
		verfication.setStudent(student);
